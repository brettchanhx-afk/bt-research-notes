...
def DelBenchmark(target_dir: str, file_name: str):

    logger.info("load all.text.\n")

    with open(rf"{target_dir}\all.txt", "r") as file:

        source_value: List = file.readlines()

    logger.info("create pool.text.\n")

    file_path: str = rf"{target_dir}\{file_name}.txt"
    my_file: Path = Path(file_path)
    ...
    with open(rf"{target_dir}\{file_name}.txt", "a") as file:

        for i in source_value[1:]:
